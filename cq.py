...
srs = [bls12_381.G1 * pow(x,i,p) for i in range(N)] #1
srs2 = [bls12_381.G2 * pow(x,i,p) for i in range(N+1)] #1
Z_V_x_2 = srs2[-1]-srs2[0] #2
T = util.inv_dft(table,alpha_N,p) #3
T_x_2 = sum((srs2[i] * T[i] for i in range(N)), start=ec.Point(None,None)) #3
K_i_x_1 = util.dft(util.toeplitz_mult(T[1:]+[0]*N,srs[::-1],alpha_2N,p),alpha_N,p) #4a
Q_i_x_1 = [K_i_x_1[i] * util.mult_inv(N*pow(alpha_N,i*(N-1),p),p) % p for i in range(N)] #4a
L_i_x_1 = util.inv_dft(srs,alpha_N,p) #4b
...
B_0 = B[1:]+[0] #6
B_0_x_1 = sum((srs[i] * B_0[i] for i in range(n)), start=ec.Point(None,None)) #7
